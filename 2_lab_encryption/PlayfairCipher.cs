    ...
    public class PlayfairCipher : ICipher
    {
		...
		private static string RemoveAllDuplicates(string str, List<int> indexes)
		{
			...
			for (int i = indexes.Count - 1; i >= 1; i--)
				retValue = retValue.Remove(indexes[i], 1);
		...
		{
			StringBuilder retValue = new StringBuilder(output);

			for (int i = 0; i < input.Length; ++i)
			{
				if (!char.IsLetter(input[i]))
					retValue = retValue.Insert(i, input[i].ToString());

				if (char.IsLower(input[i]))
					retValue[i] = char.ToLower(retValue[i]);
			}

			return retValue.ToString();
		}

		private static string Cipher(string input, string key, bool encrypting)
		{
			string retValue = string.Empty;
			char[,] keySquare = GenerateKeySquare(key);
			string tempInput = RemoveOtherChars(input);
			int e = encrypting ? 1 : -1;
			...
			for (int i = 0; i < tempInput.Length; i += 2)
			{
				...
				{
					retValue += new string(SameRowColumn(ref keySquare, row1, col1, e));
				}
				else if (row1 == row2)
				{
					retValue += new string(SameRow(ref keySquare, row1, col1, col2, e));
				}
				else if (col1 == col2)
				{
					retValue += new string(SameColumn(ref keySquare, col1, row1, row2, e));
				}
				else
				{
					retValue += new string(DifferentRowColumn(ref keySquare, row1, col1, row2, col2));
				}
			}

			retValue = AdjustOutput(input, retValue);
